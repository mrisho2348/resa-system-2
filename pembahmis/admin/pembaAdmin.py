...
from pembahmis.models import (
    PembaChiefComplaint, PembaCompany, PembaConsultationNotes, PembaDiagnosis, Country,  
     PembaHealthRecord, PembaPatientVital,
    PathodologyRecord, PembaMedicine, PembaPatient, PembaPatientVisits, 
    PembaPrescription, PembaProcedure, PembaReagent, PembaReferral, PembaService,
    Staffs, PembaObservationRecord, PembaCounseling, PembaDischargesNotes,
    PembaEquipment, PembaLaboratoryRequest, PembaAppointment
)
from clinic.models import ClinicCompany, CustomUser, DiseaseRecode
...
def pemba_dashboard(request):
    ...
    return render(request, "pembaadmin_template/home_content.html", context)
...
def fetch_procedure_order_counts(request):
    ...
    pending_procedure_count = PembaProcedure.objects.filter(
...
def fetch_laboratory_order_counts(request):
    ...
    pending_lab_count = PembaLaboratoryRequest.objects.filter(
        Q(result__isnull=True) | Q(result='')
    ).count()
...
def patient_statistics(request):
    ...
    return render(request, 'pembaadmin_template/reports_comprehensive.html', context)

# ==================== PATIENT MANAGEMENT VIEWS ====================

@login_required
def pemba_patient_list_view(request):
    patients = PembaPatient.objects.select_related('company', 'nationality', 'data_recorder').all().order_by('-created_at')
    context = {'patients': patients}
    return render(request, 'pembaadmin_template/kahama_patient_list.html', context)

@require_POST
@csrf_exempt
def delete_pemba_patient_view(request):
...
def individual_visit(request, patient_id):
    ...
    return render(request, 'pembaadmin_template/reports_individual_visit.html', context)
...
def admin_profile(request):
    ...
    try:
        ...
        return render(request, 'pembaadmin_template/profile.html', {'staff': staff})
    except Staffs.DoesNotExist:
        return render(request, 'pembaadmin_template/profile.html', {'error': 'Admin not found.'})
...
def change_password(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            logout(request)
            return redirect('pembahmis:pemba_login')
        else:
            messages.error(request, "Please correct the error below.")
    else:
        form = PasswordChangeForm(request.user)
    return render(request, 'pembaadmin_template/change_password.html', {'form': form})
...
def expired_medicine_view(request):
    ...
    return render(request, 'pembaadmin_template/expired_medicine.html', {'medicines': medicines})
...
def instock_medicine_view(request):
    ...
    return render(request, 'pembaadmin_template/instock_medicine.html', {'medicines': medicines})

@login_required
def checklist_medicine_view(request):
    medicines = PembaMedicine.objects.filter(is_clinic_stock=False)
    return render(request, 'pembaadmin_template/checklist_medicine.html', {'medicines': medicines})
...
def outofstock_medicine_view(request):
    ...
    return render(request, 'pembaadmin_template/outofstock_medicine.html', {'medicines': medicines})

@login_required
def pembamedicine_list(request):
    ...
    return render(request, 'pembaadmin_template/pembamedicine_list.html', {'medicines': medicines, 'today': date.today()})

@login_required
@csrf_exempt
def add_pemba_medicine(request):
    if request.method == 'POST':
        try:
            post = request.POST
            ...
            if drug_id:
                try:
                    medicine = PembaMedicine.objects.get(pk=drug_id)
                except PembaMedicine.DoesNotExist:
                    return JsonResponse({'success': False, 'message': 'Medicine not found for editing.'})

                if PembaMedicine.objects.exclude(pk=drug_id).filter(drug_name=drug_name).exists():
                    return JsonResponse({'success': False, 'message': 'Another medicine with the same name already exists'})
                if PembaMedicine.objects.exclude(pk=drug_id).filter(batch_number=batch_number).exists():
...
def medicine_count_api(request):
    ...
    expired_count = PembaMedicine.objects.filter(is_clinic_stock=True, expiration_date__lt=today).count()
    instock_count = PembaMedicine.objects.filter(is_clinic_stock=True, quantity__gt=0, expiration_date__gte=today).count()
    checklist_count = PembaMedicine.objects.filter(is_clinic_stock=False).count()
    outofstock_count = PembaMedicine.objects.filter(is_clinic_stock=True, quantity__lte=0).count()
    total_count = PembaMedicine.objects.all().count()
...
@csrf_exempt
@login_required
def delete_pemba_medicine(request):
    if request.method == 'POST':
        medicine_id = request.POST.get('medicine_id')
        try:
            medicine = get_object_or_404(PembaMedicine, pk=medicine_id)
            medicine.delete()
...
@login_required
def pemba_equipment_list(request):
    equipment_list = PembaEquipment.objects.all()
    return render(request, 'pembaadmin_template/pemba_equipment_list.html', {'equipment_list': equipment_list})

@login_required    
@csrf_exempt
def add_or_edit_pemba_equipment(request):
    if request.method == 'POST':
        data = request.POST
        ...
        try:
            if equipment_id:
                equipment = PembaEquipment.objects.get(id=equipment_id)
                if PembaEquipment.objects.exclude(id=equipment_id).filter(name=name).exists():
                    return JsonResponse({'success': False, 'message': 'Equipment with this name already exists.'})
                if PembaEquipment.objects.exclude(id=equipment_id).filter(serial_number=serial_number).exists():
                ...
                if PembaEquipment.objects.filter(name=name).exists():
                    return JsonResponse({'success': False, 'message': 'Equipment with this name already exists.'})
                if PembaEquipment.objects.filter(serial_number=serial_number).exists():
                    return JsonResponse({'success': False, 'message': 'Equipment with this serial number already exists.'})
                PembaEquipment.objects.create(
                    name=name,
        ...
        except PembaEquipment.DoesNotExist:
...
@login_required
@csrf_exempt
def delete_pemba_equipment(request):
    if request.method == 'POST':
...
@login_required
def pemba_reagent_list(request):
    reagent_list = PembaReagent.objects.all()
    return render(request, 'pembaadmin_template/pemba_reagent_list.html', {'reagent_list': reagent_list})

@login_required    
@csrf_exempt
def add_or_edit_pemba_reagent(request):
    if request.method == 'POST':
        data = request.POST
        ...
        try:
            if reagent_id:
                reagent = PembaReagent.objects.get(id=reagent_id)
                if PembaReagent.objects.exclude(id=reagent_id).filter(name=name).exists():
                ...
                if PembaReagent.objects.filter(name=name).exists():
                    return JsonResponse({'success': False, 'message': 'A reagent with the same name already exists.'})
                PembaReagent.objects.create(
                    name=name,
                    supplier=supplier,
                    quantity=quantity,
...
@login_required
@csrf_exempt
def delete_pemba_reagent(request):
    if request.method == 'POST':
        ...
        except PembaReagent.DoesNotExist:
...
def manage_country(request):
    ...
    return render(request, "pembaadmin_template/manage_country.html", {"countries": countries})

@login_required
def manage_company(request):
    companies = PembaCompany.objects.all() 
    return render(request, "pembaadmin_template/manage_company.html", {"companies": companies})

@login_required
def manage_disease(request):
    disease_records = DiseaseRecode.objects.all() 
    return render(request, "pembaadmin_template/manage_disease.html", {"disease_records": disease_records})

@login_required
def manage_staff(request):     
    staffs = Staffs.objects.all()  
    return render(request, "pembaadmin_template/manage_staff.html", {"staffs": staffs})  
...
def manage_pathodology(request):
    ...
    return render(request, "pembaadmin_template/manage_pathodology.html", {
        "pathodology_records": pathodology_records,
    })

@login_required
def pembaservice_list(request):
    services = PembaService.objects.all()
    return render(request, 'pembaadmin_template/service_list.html', {'services': services})

@login_required
def health_record_list(request):
    records = PembaHealthRecord.objects.all()
    return render(request, 'pembaadmin_template/healthrecord_list.html', {'records': records})
...
def patient_procedure_view(request):
    ...
    return render(request, 'pembaadmin_template/manage_procedure.html', context)

@login_required
def manage_referral(request):
    referrals = PembaReferral.objects.all()   
    return render(request, 'pembaadmin_template/manage_referral.html', {'referrals': referrals})
...
def appointment_list_view(request):
    ...
    return render(request, 'pembaadmin_template/manage_appointment.html', context)
...
def prescription_list(request):
    ...
    return render(request, 'pembaadmin_template/manage_prescription_list.html', {
...
def consultation_notes_view(request):
    patient_records = (
        PembaPatient.objects.filter(consultation_notes__isnull=False)
        .distinct()
        .order_by('-consultation_notes__updated_at')
    )

    return render(request, 'pembaadmin_template/manage_consultation_notes.html', {
        'patient_records': patient_records
    })

@login_required
def diagnosis_list(request):
    diagnoses = PembaDiagnosis.objects.all().order_by('-created_at')    
    return render(request, 'pembaadmin_template/manage_diagnosis_list.html', {'diagnoses': diagnoses}) 

@login_required
def counseling_list_view(request):
    counselings = PembaCounseling.objects.all().order_by('-created_at')
    return render(request, 'pembaadmin_template/manage_counselling.html', {'counselings': counselings})    

@login_required
def observation_record_list_view(request):
    observation_records = PembaObservationRecord.objects.all().order_by('-created_at')
    return render(request, 'pembaadmin_template/manage_observation_record.html', {'observation_records': observation_records})

@login_required
def discharge_notes_list_view(request):
    discharge_notes = PembaDischargesNotes.objects.all().order_by('-discharge_date')
    return render(request, 'pembaadmin_template/manage_discharge.html', {'discharge_notes': discharge_notes})
...
def patient_laboratory_view(request):
    ...
    return render(request, 'pembaadmin_template/manage_lab_result.html', context)    
...
def reports_by_visit(request):
    ...
    return render(request, 'pembaadmin_template/reports_by_visit.html', context)

@login_required
def reports_patients_visit_summary(request):
    visits = PembaPatientVisits.objects.all()
    context = {'visits': visits}
    return render(request, "pembaadmin_template/reports_patients_visit_summary.html", context)

@login_required
def reports_patients(request):
    patients_report = PembaPatient.objects.order_by('-created_at') 
    context = {'patients': patients_report}
    return render(request, "pembaadmin_template/reports_patients.html", context)

@login_required
def resa_report(request):
    return render(request,"pembaadmin_template/resa_reports.html")    
...
def search_report(request):
    if request.method == 'POST' and request.META.get('HTTP_X_REQUESTED_WITH') == 'XMLHttpRequest':
        ...
        report_templates = {
            'patient_type_reports': 'pembaadmin_template/patient_type_report_table.html',
            'patient_company_wise_reports': 'pembaadmin_template/company_wise_reports_table.html',
            'patient_lab_result_reports': 'pembaadmin_template/laboratory_report_table.html',
            'patient_procedure_reports': 'pembaadmin_template/procedure_report_table.html',
            'patient_referral_reports': 'pembaadmin_template/referral_reports_table.html',
            'patient_pathology_reports': 'pembaadmin_template/pathology_record_report_table.html',
        }
...
def render_report(report_type, year):
    if report_type == 'patient_type_reports':       
        ...
        return render_to_string('pembaadmin_template/patient_type_report_table.html', context)
        ...
        return render_to_string('pembaadmin_template/company_wise_reports_table.html', context)
        ...
        return render_to_string('pembaadmin_template/laboratory_report_table.html', context)
        ...
        return render_to_string('pembaadmin_template/procedure_report_table.html', context)
        ...
        return render_to_string('pembaadmin_template/referral_reports_table.html', context)
        ...
        return render_to_string('pembaadmin_template/pathology_record_report_table.html',context)
...
@require_POST
def delete_pemba_company(request):
    company_id = request.POST.get('company_id')
    try:
        PembaCompany.objects.get(id=company_id).delete()
        return JsonResponse({'success': True})
    except PembaCompany.DoesNotExist:
...
def add_company(request):
    if request.method == 'POST':
        try:
            ...
            if company_id:
                try:
                    company = PembaCompany.objects.get(pk=company_id)
                    if PembaCompany.objects.filter(name=name).exclude(pk=company_id).exists():
                ...
                if PembaCompany.objects.filter(name=name).exists():
                    return JsonResponse({'success': False, 'message': 'Company already exists.'})
                PembaCompany.objects.create(name=name)
...
def save_diagnosis(request):
    try:
        ...
        if diagnosis_id:
            try:
                diagnosis = PembaDiagnosis.objects.get(pk=diagnosis_id)
            except PembaDiagnosis.DoesNotExist:
                return JsonResponse({'success': False, 'message': 'Diagnosis not found.'})

            if PembaDiagnosis.objects.exclude(pk=diagnosis_id).filter(diagnosis_name=diagnosis_name).exists():
                return JsonResponse({'success': False, 'message': 'Diagnosis with this name already exists.'})
            if PembaDiagnosis.objects.exclude(pk=diagnosis_id).filter(diagnosis_code=diagnosis_code).exists():
            ...
            if PembaDiagnosis.objects.filter(diagnosis_name=diagnosis_name).exists():
                return JsonResponse({'success': False, 'message': 'Diagnosis with this name already exists.'})
            if PembaDiagnosis.objects.filter(diagnosis_code=diagnosis_code).exists():
                return JsonResponse({'success': False, 'message': 'Diagnosis with this code already exists.'})

            PembaDiagnosis.objects.create(
...
@csrf_exempt
@require_POST
def save_pemba_service(request):
    try:
        ...
        if service_id:
            service = PembaService.objects.get(pk=service_id)
            if PembaService.objects.exclude(pk=service_id).filter(name=name, data_recorder=data_recorder).exists():
            ...
            if PembaService.objects.filter(name=name).exists():
                return JsonResponse({'success': False, 'message': f'A service with the name "{name}" already exists.'})
            PembaService.objects.create(
                name=name, 
                description=description, 
                category=category,
    ...
    except PembaService.DoesNotExist:
    ...
    if request.method == 'POST':
        try:
            ...
            if health_record_id:
                ...
                if PembaHealthRecord.objects.exclude(pk=health_record_id).filter(name=name).exists():
                ...
                if PembaHealthRecord.objects.filter(name=name).exists():
                    return JsonResponse({'success': False, 'message': f'A record with the name "{name}" already exists.'})
                PembaHealthRecord.objects.create(
                    name=name, 
                    data_recorder=request.user.staff
                )
...
def company_registration_view(request):
    try:
        ...
        if request.method == 'POST':
            ...
            return render(request, 'pembaadmin_template/company_registration.html', {'company': company})
    except Exception as e:
        messages.error(request, f'An error occurred: {str(e)}')
        return render(request, 'pembaadmin_template/company_registration.html')
...
def delete_consultation(request):
    if request.method == 'POST':
        ...
        try:
            appointment = PembaAppointment.objects.get(id=appointment_id)
            appointment.delete()
            return JsonResponse({'status': 'success'})
        except PembaAppointment.DoesNotExist:
...
def delete_observation(request):
    if request.method == 'POST':
        try:
            ...
            observation = PembaObservationRecord.objects.get(id=observation_id)
            observation.delete()
            return JsonResponse({'success': True, 'message': f'Observation record deleted successfully.'})
        except PembaObservationRecord.DoesNotExist:
...
def delete_lab_result(request):
    if request.method == 'POST':
        try:
            ...
            lab_result = PembaLaboratoryRequest.objects.get(id=lab_result_id)
            lab_result.delete()
            return JsonResponse({'success': True, 'message': f'Lab result record deleted successfully.'})
        except PembaLaboratoryRequest.DoesNotExist:
...
def delete_pemba_company(request):
    if request.method == 'POST':
        ...
        try:
            company = get_object_or_404(PembaCompany, pk=company_id)
            company.delete()
            return JsonResponse({'success': True})
        except Exception as e:
...
def delete_service(request):
    try:
        service_id = request.POST.get('service_id')
        service = PembaService.objects.get(id=service_id)
        service.delete()
...
def delete_procedure(request):
    if request.method == 'POST':
        ...
        try:
            procedure = PembaProcedure.objects.get(id=procedure_id)
            procedure.delete()
...
def delete_discharge_note(request):
    if request.method == 'POST':
        try:
            observation_id = request.POST.get('observation_id')
            observation = PembaDischargesNotes.objects.get(id=observation_id)
            observation.delete()
            return JsonResponse({'success': True, 'message': f'Discharge record deleted successfully.'})
        except PembaDischargesNotes.DoesNotExist:
            return JsonResponse({'success': False, 'message': 'Invalid observation ID.'})
        except Exception as e:
            return JsonResponse({'success': False, 'message': f'An error occurred: {e}'})
    return JsonResponse({'success': False, 'message': 'Invalid request method.'})
...
def delete_counseling_session(request):
    if request.method == 'POST':
        reagent_id = request.POST.get('id')
        try:
            reagent = PembaCounseling.objects.get(id=reagent_id)
            reagent.delete()
            return JsonResponse({'status': 'true', 'message': 'Counsel record deleted successfully'})
        except PembaCounseling.DoesNotExist:
...
def delete_diagnosis(request):
    try:
        diagnosis_id = request.POST.get('diagnosis_id')
        diagnosis = get_object_or_404(PembaDiagnosis, pk=diagnosis_id)
        diagnosis.delete()
        return JsonResponse({'status': 'success'})
    except Exception as e:
        return JsonResponse({'status': 'error', 'message': str(e)})
...
def download_referral_pdf(request, patient_id, visit_id):
    ...
    html_content = render_to_string('pemba_template/view_referral.html', context)
...
def download_consultation_summary_pdf(request, patient_id, visit_id):
    ...
    html_content = render_to_string('pemba_template/pdf_consultation_summary.html', context)
...
def download_counseling_pdf(request, patient_id, visit_id):
    ...
    html_content = render_to_string('pemba_template/counseling_notes_details.html', context)
    temp_dir = os.path.join(os.path.expanduser("~"), "pdf_temp")
    os.makedirs(temp_dir, exist_ok=True)
...
def download_discharge_pdf(request, patient_id, visit_id):
    ...
    html_content = render_to_string('pemba_template/discharge_note_detail.html', context)
    temp_dir = os.path.join(os.path.expanduser("~"), "pdf_temp")
    os.makedirs(temp_dir, exist_ok=True)
...
def download_lab_result_pdf(request, lab_id):
    ...
    html_content = render_to_string('pemba_template/pdf_lab_result.html', context)
...
def download_all_lab_results_pdf(request, patient_mrn, visit_vst):
    ...
    html_content = render_to_string('pemba_template/pdf_all_lab_results.html', context)
...
def download_observation_pdf(request, patient_id, visit_id):
    ...
    html_content = render_to_string('pemba_template/observation_notes_detail.html', context)
...
def download_prescription_notes_pdf(request, patient_id, visit_id):
    ...
    html_content = render_to_string('pemba_template/prescription_notes.html', context)
...
def download_procedure_result_pdf(request, procedure_id):
    ...
    html_content = render_to_string('pemba_template/pdf_procedure_result.html', context)
    temp_dir = os.path.join(os.path.expanduser("~"), "pdf_temp")
    os.makedirs(temp_dir, exist_ok=True)
...
def download_all_procedures_pdf(request, patient_mrn, visit_vst):
    ...
    html_content = render_to_string('pemba_template/pdf_all_procedures.html', context)
