...
from kahamahmis.models import KahamaCompany, KahamaLaboratoryRequest, KahamaPatient, KahamaProcedure, KahamaReferral, KahamaService
...
from clinic.models import PathodologyRecord
...
def render_patient_referral_reports(sheet, year):
    ...
    for idx, referral in enumerate(referrals, start=3):  # Note: Starting from row 3
        ...
        diagnosis_content = ''
        for diagnosis_record in referral.patient.diagnosis_records.all():
...
def fetch_daily_patient_type_wise_reports(year, month):
    ...
    patients_by_type = (
        KahamaPatient.objects.filter(created_at__year=year, created_at__month=month)
...
def fetch_daily_pathology_reports(year, month):
    ...
    patients_by_pathology_record = (
        PathodologyRecord.objects.filter(kahamaconsultationnotes__created_at__year=year, kahamaconsultationnotes__created_at__month=month)
        .annotate(day=TruncDay('kahamaconsultationnotes__created_at'))
        .values('name', 'day')
        .annotate(num_patients=Count('kahamaconsultationnotes__id'))
    )
...
def fetch_daily_procedure_reports(year, month):
    ...
    procedures_by_date = (
        KahamaProcedure.objects.filter(created_at__year=year, created_at__month=month)
        .annotate(day=TruncDay('created_at'))
...
def fetch_daily_patient_laboratory_reports(year, month):
    ...
    laboratories_by_date = (
        KahamaLaboratoryRequest.objects.filter(created_at__year=year, created_at__month=month)
...
def fetch_daily_patient_referral_reports(year, month):
    ...
    referrals = KahamaReferral.objects.filter(created_at__year=year, created_at__month=month)
    return referrals
